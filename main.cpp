...
void drawProgressBar(int x, int y, int width, int height, int killCount);
...
void updateWindow() {
	...
	drawProgressBar(480,500,300,50,killCount);
	EndBatchDraw(); //结束双缓冲
}
...
void drawProgressBar(int x, int y, int width, int height, int zmCount) {
	// 背景条
	setlinecolor(WHITE);
	...
	// 填充进度
	double ratio = (double)zmCount / ZM_MAX;
	int fillWidth = static_cast<int>(ratio * width);

	setfillcolor(RGB(0, 200, 0));  // 植物绿
	fillrectangle(x, y, x + fillWidth, y + height);

	// 文字提示
	settextcolor(WHITE);
	setbkmode(TRANSPARENT);
	std::string text = "Zombie Progress: " + std::to_string(zmCount) + " / " + std::to_string(ZM_MAX);
	outtextxy(x + 10, y + height / 4, text.c_str());
